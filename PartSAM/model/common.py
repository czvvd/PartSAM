...
def sample_prompts( 
    ...
    prompt_coords, prompt_labels, selected_indices = [], [], []
    for i in range(batch_size):
        for j in range(num_masks):
            ...
            if len(diff_inds) < num_samples:
                ...
                pad_size = num_samples - len(diff_inds)
                pad_inds = diff_inds[torch.randint(0, len(diff_inds), (pad_size,))]
                diff_inds = torch.cat([diff_inds, pad_inds], dim=0)
    ...
    if return_idx:
        return prompt_coords, prompt_labels, selected_indices
    else:
...
def sample_fixed_points(
    ...
    if from_error_region:
        ...
        for i in range(batch_size):
            for j in range(num_masks):
                if return_idx:
                    coords, label, _, selected_idx = sample_furthest_points_from_border(
                        points[i], mask[i, j], gt_masks[i, j], return_idx=return_idx
                    )
...
def sample_near_points(
    ...
    for i in range(batch_size):
        for j in range(num_masks):
            if return_idx:
                coords, label, _, selected_idx = sample_near_points_from_border(
                    points[i], mask[i, j], gt_masks[i, j], return_idx=return_idx
                )
                prompt_points.append(coords)
                prompt_labels.append(label)
                ...
                coords, label, _ = sample_near_points_from_border(
                    points[i], mask[i, j], gt_masks[i, j], return_idx=return_idx
                )
                prompt_points.append(coords)
                prompt_labels.append(label)
...
def sample_near_points_from_border(
    coords: torch.Tensor, lables: torch.Tensor, gt: torch.Tensor, return_idx=False, partition=3
):
    """
    Sample points from the border of the mask.
    ...
    # if bg_inds or fg_inds is empty, return None
    if bg_inds.sum().item() == 0 or fg_inds.sum().item() == 0:
        # Instead of returning None, return a random point from coords
        rand_idx = torch.randint(0, coords.shape[0], (1,))
        return coords[rand_idx], gt[rand_idx], -1, rand_idx
        # return None,None,-1

    # All distances from foreground points to background points
    min_dists, _ = chamfer_distance(coords[fg_inds][None, ...], coords[bg_inds][None, ...])
    # Sample randomly from the farthest half of the points from the border
    half = max(1, min_dists.shape[1] // partition)
    topk_vals, topk_indices = torch.topk(min_dists.squeeze(0), half, largest=True)
    rand_idx = random.randint(0, half - 1)
    center_idx = topk_indices[rand_idx]
    # center_idx = torch.argmax(min_dists)
    # center_coords = coords[fg_inds][center_idx]
    fg_indices = torch.nonzero(fg_inds, as_tuple=False).squeeze(1)
    orig_center_idx = fg_indices[center_idx]
...
def sample_PC(
    ...
    # center_coords = coords[fg_inds][center_idx]
    fg_indices = torch.nonzero(fg_inds, as_tuple=False).squeeze(1)
    if topk_indices.numel() < sample_num // 2:
        pad_size = sample_num // 2 - topk_indices.numel()
        pad_inds = topk_indices[torch.randint(0, topk_indices.numel(), (pad_size,))]
        topk_indices = torch.cat([topk_indices, pad_inds], dim=0)
    orig_center_idx = fg_indices[topk_indices]
    center_coords2 = coords[orig_center_idx]

    # 采样前景点，如果数量不足则pad
    if fg_indices.numel() < sample_num // 2:
        pad_size = sample_num // 2 - fg_indices.numel()
