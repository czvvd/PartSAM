...
https://github.com/Pointcept/Pointcept/blob/df36980119f4636beb2d02d04ef3b2fec0fddfba/pointcept/datasets/transform.py
...
class ToTensor(object):
    def __call__(self, data):
        if isinstance(data, torch.Tensor):
            ...
            return result
        elif isinstance(data, Sequence) and not isinstance(data[0], str):
...
class NormalizeMy(object):
    def __call__(self, data_dict):
        if "coord" in data_dict.keys():
            ...
            data_dict["coord"] = (coord - center) * scale
            if data_dict["vertices"] is not None:
                data_dict["vertices"] = (data_dict["vertices"] - center) * scale
        return data_dict
...
class RandomDropout(object):
    ...
    def __call__(self, data_dict):
        if random.random() < self.dropout_application_ratio:
            ...
            if "point2face" in data_dict.keys():
                data_dict["point2face"] = data_dict["point2face"][idx]
            if "mask2pt" in data_dict.keys():
                data_dict["mask2pt"] = data_dict["mask2pt"][:,idx] # axis1 is points
        return data_dict
...
class RandomRotate(object):
    ...
    def __call__(self, data_dict):
        ...
        if "vertices" in data_dict.keys():
            data_dict["vertices"] -= center
            data_dict["vertices"] = np.dot(data_dict["vertices"], np.transpose(rot_t))
            data_dict["vertices"] += center
        return data_dict
...
class RandomFlip(object):
    ...
    def __call__(self, data_dict):
        if np.random.rand() < self.p:
            ...
            if "vertices" in data_dict.keys():
                data_dict["vertices"][:, 0] = -data_dict["vertices"][:, 0]
        if np.random.rand() < self.p:
            if "coord" in data_dict.keys():
            ...
            if "vertices" in data_dict.keys():
                data_dict["vertices"][:, 1] = -data_dict["vertices"][:, 1]
        return data_dict
...
class GridSample(object):
    ...
    def __call__(self, data_dict):
        ...
        if self.mode == "train":  # train mode
            ...
            if "point2face" in data_dict.keys():
                data_dict["point2face"] = data_dict["point2face"][idx_unique]
            if "mask2pt" in data_dict.keys():
                data_dict["mask2pt"] = data_dict["mask2pt"][:,idx_unique] # axis1 is points
            if "gt" in data_dict.keys():
                data_dict["gt"] = data_dict["gt"][idx_unique]
            #data_dict["gridsample_idxs"] = idx_unique
            return data_dict
            ...
            for i in range(count.max()):
                ...
                for key in data_dict.keys():
                    ...
                    elif "point2face" in data_dict.keys():
...
class SphereCrop(object):
    ...
    def __call__(self, data_dict):
        ...
        # mode is "random" or "center"
            ...
            if "point2face" in data_dict.keys():
                data_dict["point2face"] = data_dict["point2face"][idx_crop]
            if "mask2pt" in data_dict.keys():
                data_dict["mask2pt"] = data_dict["mask2pt"][:,idx_crop] # axis1 is points
        return data_dict
